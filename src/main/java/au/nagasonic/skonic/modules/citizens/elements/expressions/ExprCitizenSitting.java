...
@Name("Citizen Sitting")
@Description("Whether a Citizens NPC is sitting.")
@RequiredPlugins("Citizens")
@Since("1.3")
@Examples("set sitting state of {_npc} to true")
public class ExprCitizenSitting extends SimplePropertyExpression<NPC, Boolean> {
    public static void register(SyntaxRegistry syntaxRegistry) {
        syntaxRegistry.register(
            SyntaxRegistry.EXPRESSION,
            DefaultSyntaxInfos.Expression.builder(ExprCitizenSitting.class, Boolean.class)
                .addPatterns(
    ...
    public @Nullable Boolean convert(NPC npc) {
        if (npc.hasTrait(SitTrait.class)) {
            return npc.getOrAddTrait(SitTrait.class).isSitting();
        }
        return false;
    }
    ...
    public void change(Event event, @Nullable Object[] delta, Changer.ChangeMode mode) {
        if (delta != null && delta[0] instanceof Boolean) {
            boolean sitting = (Boolean) delta[0];
