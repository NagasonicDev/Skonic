...
@Since("1.3")
@Examples("set npc knockback of {_npc} to true")
public class ExprCitizenKnockback extends SimplePropertyExpression<NPC, Boolean> {
    public static void register(SyntaxRegistry syntaxRegistry) {
        syntaxRegistry.register(
            SyntaxRegistry.EXPRESSION,
            DefaultSyntaxInfos.Expression.builder(ExprCitizenKnockback.class, Boolean.class)
                .addPatterns(
    ...
    public @Nullable Boolean convert(NPC npc) {
        return npc.data().get(NPC.Metadata.KNOCKBACK, false);
    }
    ...
    public void change(Event event, @Nullable Object[] delta, Changer.ChangeMode mode) {
        if (delta != null && delta[0] instanceof Boolean) {
            boolean knockback = (Boolean) delta[0];
            for (NPC npc : getExpr().getArray(event)) {
                npc.data().setPersistent(NPC.Metadata.KNOCKBACK, knockback);
            }
        }
    }
