...
@Name("Citizen Wander")
@Description("Whether a Citizens NPC has the Waypoints trait (wandering).")
@RequiredPlugins("Citizens")
@Since("1.3")
@Examples("set wandering of {_npc} to true")
public class ExprCitizenWander extends SimplePropertyExpression<NPC, Boolean> {
    public static void register(SyntaxRegistry syntaxRegistry) {
        syntaxRegistry.register(
            SyntaxRegistry.EXPRESSION,
            DefaultSyntaxInfos.Expression.builder(ExprCitizenWander.class, Boolean.class)
                .addPatterns(
    ...
    public @Nullable Boolean convert(NPC npc) {
        return npc.hasTrait(Waypoints.class);
    }
    ...
    public void change(Event event, @Nullable Object[] delta, Changer.ChangeMode mode) {
        if (delta != null && delta[0] instanceof Boolean) {
            boolean wander = (Boolean) delta[0];
