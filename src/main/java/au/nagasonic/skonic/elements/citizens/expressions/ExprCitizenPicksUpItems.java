...
@Since("1.3")
@Examples("set npc picks up items of {_npc} to true")
public class ExprCitizenPicksUpItems extends SimplePropertyExpression<NPC, Boolean> {
    public static void register(SyntaxRegistry syntaxRegistry) {
        syntaxRegistry.register(
            SyntaxRegistry.EXPRESSION,
            DefaultSyntaxInfos.Expression.builder(ExprCitizenPicksUpItems.class, Boolean.class)
                .addPatterns(
    ...
    public @Nullable Boolean convert(NPC npc) {
        return npc.data().get(NPC.Metadata.PICKUP_ITEMS, false);
    }
    ...
    public void change(Event event, @Nullable Object[] delta, Changer.ChangeMode mode) {
        if (delta != null && delta[0] instanceof Boolean) {
            boolean pickup = (Boolean) delta[0];
